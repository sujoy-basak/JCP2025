...
using ..SSFR: fr_dir, lwfr_dir, eq_dir, src_dir
using ..SSFR: update_ghost_values_periodic!, flux, con2prim, check_admissibility
(
...
function compute_time_step(eq::AbstractEquations{1, 1}, grid, aux, op, cfl, u1,
   ...
   for i=1:nx
      ...
      den = max(den, abs.(sx)/dx[i] + 1.0e-20)
   end
   dt = cfl / den
   return dt
   end # Timer
...
function apply_bound_limiter!(eq::AbstractEquations{1, 1}, grid, scheme, param, op,
   ...
   for i=1:nx
      ...
      ratio1 = abs(M - ua_[i])/(abs(Mi - ua_[i]) + 1e-20)
      ratio2 = abs(m - ua_[i])/(abs(mi - ua_[i]) + 1e-20)
...
function modal_smoothness_indicator_new(eq::AbstractEquations{1}, t, iter,
   ...
   for i=1:nx
      ...
      for i=1:nd #for BLAST
         if(~is_admissible(eq,un[:,i]))
            @show i
         end
      end
      ...
      ind_den, ind_num, ind = ( zeros(n_ind_nvar), zeros(n_ind_nvar),zeros(n_ind_nvar) )
  ...
  # end
   ...
   for i=1:nx
      ...
      #=if alpha[i] < amax || true
         ss = a0*alpha0[i-1]+a1*alpha[i]+a0*alpha0[i+1]
         alpha[i] = max(ss,alpha[i])
      end=#
      if alpha[i] < amax || true
         alpha[i] = max(alpha0[i-1]/2,alpha[i],alpha0[i+1]/2)
      end
...
function debug_blend_limiter!(eq::AbstractEquations{1}, grid, problem, scheme,
   ...
   if blend.debug == false
      return nothing    # GETS OUT OF THE FUNCTION
   end
   ...
   if nvariables(eq) > 1 # KLUDGE - Create a p
      ua_min, ua_max = [1e20, 1e20, 1e20, 1e20], [-1e20, -1e20, -1e20, -1e20]
      for i=1:nx
...
@inbounds @inline function blend_cell_residual_muscl!(cell,
   ...
   # @unpack beta = blend
   beta = 1.0 #2.0 - blend.alpha[cell] #9sept #1.0 #
...
@inbounds function blend_face_residual_muscl!(i, xf, u1, ua,
   ...
   return Fn, (1.0 - alpha[i-1], 1.0 - alpha[i]) #9sept
...
function is_admissible_extreme(::AbstractEquations{1,<:Any}, ::AbstractVector, ::AbstractVector, ::AbstractVector, ::MMatrix, ::MMatrix)
   # Check if the invariant domain in preserved. This has to be
   # extended in Equation module
   return true
end
...
function Blend(eq::AbstractEquations{1}, op, grid,
   ...
   elseif indicator_model == "model5"
      ...
      E1 = 1.0 #0.873583 #0.0873583 #0.9 #8sept
   ...
   E0 = E1 * 0.001 #1e-2 # E < E0 implies smoothness #8sept
   
   tolE = 1.0e-15#1.0e-6  # If denominator < tolE, do purely high order
 ...
 ##SEE #4 sept
   ...
   for n in 0:nvar-1       ##SEE PLOTTING
...
# Compute error norm
#-------------------------------------------------------------------------------
function compute_error(problem, grid, eq::AbstractEquations{1}, aux, op, u1, t, scheme)
   ...
   println("compute_error is running")

   nq     = nd #+ 10    # number of quadrature points in each direction   ########## FOR L2 ERROR
   spd = scheme.solution_points
   xq, wq = weights_and_points(nq, spd)
   #xq, wq = weights_and_points(nq, "gl")  ##SEE CHANGE
...
global exacts=[]
   ...
   for i=1:ceil(Int64,nx) ##
      un, ue = zeros(nq), zeros(nq) # exact solution
      x = xc[i] - 0.5 * dx[i] .+ dx[i] * xq  ## FOR [0,1] TO [a,b]

      con_exact= zeros(nvar,nq)
      prim_exact= zeros(nvar,nq)
      
      for n=1:nvar
         for j=1:nq
            con_exact[n,j]=exact_solution(x[j],t)[n]
         end
      end

      for j=1:nq
         @views con2prim!(eq, con_exact[:,j],prim_exact[:,j])
         ue[j] = prim_exact[1,j] #We are considering only fluid density.
      end
      # prim_exact .= con_exact
      #con_cal= zeros(nvar, nq)
      prim_cal= zeros(nvar, nd)
      for j=1:nd
         @views con2prim!(eq, u1[:,j,i],prim_cal[:,j])
      end

      @views mul!(un, V, prim_cal[1, :])  #We are considering only fluid density.

      global exacts = [exacts..., ue...]
      #@views mul!(un, V, u1[1, :,i])

      du         = abs.(un - ue)
      linf       = maximum(du)
      l1         = dx[i] * BLAS.dot(nq, du, 1, wq, 1)  ##NO A 1/2 FACTOR THERE BECAUSE WEIGHTS ARE WI/2
     @. du      = du*du
